                    ...
                    <div class="mb-4">
                        ...
                        <select name="animal_id" id="animal_id" class="mt-1 block w-full" required>
                            @foreach($animais as $animal)
                                <option value="{{ $animal->id }}" {{ $animal->id == $adocao->animal_id ? 'selected' : '' }}>{{ $animal->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        ...
                        <select name="adotante_id" id="adotante_id" class="mt-1 block w-full" required>
                            @foreach($adotantes as $adotante)
                                <option value="{{ $adotante->id }}" {{ $adotante->id == $adocao->adotante_id ? 'selected' : '' }}>{{ $adotante->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="data_adocao" class="block text-sm font-medium text-gray-700">Data da Adoção</label>
                        <input type="date" name="data_adocao" id="data_adocao" class="mt-1 block w-full" value="{{ old('data_adocao', $adocao->data_adocao) }}" required>
                    </div>
                    <div class="mb-4">
